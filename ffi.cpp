...
inline uint64_t allocdb_size_of(uint64_t ptr){ return AllocDB::size_of(ptr); }
...
inline uint64_t get_root(AllocDB* db){ return db->root(); }
inline void set_root(AllocDB* db, uint64_t r){ db->root(r); }
