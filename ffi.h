...
inline uint64_t allocdb_size_of(uint64_t ptr);
...
inline uint64_t get_root(AllocDB* db);

// Set the root pointer. See `get_root()`.
inline void set_root(AllocDB* db, uint64_t r);